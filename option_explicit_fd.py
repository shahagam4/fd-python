...
import csv
'''
Code for option value using explicit finite difference.
PType="C" for call option and PType="P" for put option.
EType="Y" for early excercise.
NAS is number of asset steps.
Reference code page no. 1221 of book "Paul-Wilmott-on-Quantitative-Finance"
Output: The option values and greeks output.
'''

def Option_Value_2D_US(Vol, Int_Rate, PType, Strike, Expiration, EType, NAS):
    S = np.zeros((1,NAS+1))
