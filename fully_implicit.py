...
Reference page no. 1227 of book "Paul-Wilmott-on-Quantitative-Finance"
Output: The option values and greeks output.
'''

def Option_Value_3D_fi(Vol, Int_Rate, PType, Strike, Expiration, EType, NAS, NTS):
    S = np.zeros((1,NAS+1))
    ...
    for i in range(1,NAS):
        A[0, i-1]=-((Vol**2*i**2-Int_Rate*i)*dt)/2
        B[0, i-1]=((Vol**2*i**2+Int_Rate)*dt)
        C[0, i-1]=-((Vol**2*i**2+Int_Rate*i)*dt)/2
...
C=Option_Value_3D_fi(0.2,0.05,"P",100,1, "N",20,18)

with open("table_fi.csv","w+") as my_csv:
